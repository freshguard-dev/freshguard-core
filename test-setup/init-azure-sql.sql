-- Azure SQL Database initialization script for integration tests
-- Creates test tables and inserts realistic test data
-- Uses T-SQL syntax compatible with Azure SQL Database
--
-- Prerequisites:
--   1. Create an Azure SQL Database (e.g. freshguard_test)
--   2. Ensure your IP is allowed in the server firewall rules
--   3. Connect using SSMS, Azure Data Studio, or the Azure portal Query Editor
--   4. Execute this script against the freshguard_test database
--
-- Azure SQL notes:
--   - Uses IDENTITY(1,1) for auto-increment
--   - Uses DATETIME2 for timestamps, GETDATE() for current time
--   - Uses DATEADD() for interval arithmetic
